...
exports.onCreateNode = ({ node, actions, getNode }) => {
  ...
  if (node.internal.type === `MarkdownRemark` || node.internal.type === `Mdx`) {
    ...
    // create a marker for future posts  // highlight-line
    const postDate = node.frontmatter.date; // highlight-line
    const isFuturePost = !postDate || (new Date(postDate)).getTime() > Date.now() // highlight-line
    createNodeField({  // highlight-line
      name: `isFuturePost`, // highlight-line
      node,  // highlight-line
      value: isFuturePost, // highlight-line
    }) // highlight-line
  }
}
```
...
exports.createPages = async ({ graphql, actions }) => {
  ...
  const result = await graphql(`
    query {
      //...
      allMdx(filter: {fields: {isFuturePost: {eq: false}}}) { // highlight-line
        edges {
          node {
            id
            frontmatter{
...
export const pageQuery = graphql`
  query {
    ...
    allMdx(
      ...
      filter: {fields: {isFuturePost: {eq: false}}}  // highlight-line 
    ) {
      edges {
        node {
          excerpt
