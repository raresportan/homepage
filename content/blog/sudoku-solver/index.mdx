...
I became interested in the Sudoku game after a failed oportunity to build it for a client.
...
So I started to build it for myself. Soon I've found that I need:

- a solver: program that solves the game by finding all the missing digits
- a generator: program that can generate games of different difficulty, idealy automated, new games every day.
- a great UI: candidates support, lot of helper functions like hints, auto check for mistakes,
  check if a cell is correct, check if all the cells are correct and so on.

For the moment I've implemented the solver, and this post we'll detail how it was done.
If you want to test it, here it is, find a Sudoku somewhere, put the digits in and press
the "Solve" button. If the Sudoku can be solved you'll see the solution:
...
written in Python. Sadly I needed the JavaScript version, and an obvious port of that Python code
is not possible because JavaScript doesn't have the same iterators and built in functions.

The solving strategy is:

1. For each cell with single digit (known digit from the start) eliminate that digit
   from all the peer cells (the row, the column and the box that contain the digit).
2. After these initial eliminations, is possible to have new cells with single digits,
   so we need to repeat step 1 for them. In the end we'll have some cells with single digit,
   some maybe with 2 or 3 digits, some still with 9 digits - representing all the posible
   digits that might go in that cell.
3. Do a brute force search for the solution by looping trough each cell digit
   until we have the solution. (For example, using the above grid, we try 4,1,1,1...,
   then 4,6,1,1...).

## Data models

To create the data models we'll use a fuction that creates the "cross product"
of two array. Cross `[A, B]` with `[1, 2]` will result in `[A1, A2, B1, B2]`
...
Each sudoku gives us some digits, so we know from the start the digit in some squares. We can safely
remove those digits from the rest of the square peers. For example if we know that 4 is the digit
that goes in the first cell, A1, then we can remove 4 from all the row, column and box squares that
contain A1:
...
Afte this phase is done, `solve` function does a brute force search for a solution.
