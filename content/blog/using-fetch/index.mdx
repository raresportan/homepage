...
```JavaScript
fetch(url) 
    .then(response => { // highlight-line
        if(!response.ok) { // highlight-line
            const error = new Error(response.statusText || 'Fetch failed') // highlight-line
            error.response = response; // highlight-line
            throw error; // highlight-line
        } // highlight-line
        return response; // highlight-line
    }) // highlight-line
...
fetch(url) 
    .then(checkForErrors) // highlight-line
    .then(response => response.json() // this is skipped for 4xx-5xx errors!
    .catch(error => {
        // this is now called for 4xx-5xx errors!
    });
...
fetch(url, 
    {  
        method : 'POST'  // highlight-line
        headers: { // highlight-line
            'Content-Type': 'application/json', // highlight-line
        }, // highlight-line
        body: JSON.stringify(data), // highlight-line
    }) 
    .then(response => {})
    .catch(error => {});
```
...
fetch(url, 
    {  
       credentials: 'include' // highlight-line
    }) 
    .then(response => {})
    .catch(error => {});
```
...
fetchTimeout(url, {}, 10000) // highlight-line
...
fetch(url, 
    ...
    .then(response => {})
    .catch(error => {
        if (error.name === 'AbortError') { // highlight-line
            console.log('Fetch aborted'); // highlight-line
        } else {
