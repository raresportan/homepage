...
and I cancel the deploy. I'm thinking maybe there was a glitch in Netlify bowels and I start a new one. I wait again... Finally after 13 minutes the build finishes.


> I'm on the free plan (Starter) on Netlify. It includes 300 minutes/30 days of build time. For all my account websites.

I run the build on my local machine, the same build runs in 2 minutes so clearly something is very wrong somewhere. So I look at the Netlify build logs:

The first weird thing is that the cache is huge:
![Cache is huge](./cache_log.png "Cache is huge")

On my local machine, the `.cache` + `public` folders are under 60Mb. Where are the other 900Mb coming from? After further investigation, I've found that this cache keeps other things also, like the `node_modules` 😅
...
## Clear the cache
To get rid of the huge cache you need to go to your site page on Netlify, choose Deploys, select the last deploy, and from the "Retry deploy" dropdown, redeploy the site with the "Clear cache and redeploy site" option:
...
I've removed it, and from my website's Plugins page, I've added "Essential Gatsby" plugin:
![Plugins](./plugins.png "Plugins")

> If you use this plugin, the cache will be created again, but a bit smaller this time.

After these changes, the build time is back to 3 minutes. 
