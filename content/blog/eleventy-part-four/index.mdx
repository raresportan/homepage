...
![Destkop](./halfscreen.png "Desktop version")
* On mobile, the image is displayed full-screen width
![Mobile](./fullscreen.png "Mobile version")
...
async function createImages(src) {
  ...
  const metadata = await Image(src, {
      ...
      formats: ["avif", "webp", "jpeg"],
      filenameFormat: function (id, src, width, format, options) { // highlight-line
...
{ avif:
   [ { format: 'avif',
       width: 320,
       height: 213,
       filename: 'dog_small.avif',  // highlight-line
     ...
     { format: 'avif',
       width: 640,
       height: 426,
       filename: 'dog_medium.avif', // highlight-line
     ...
     { format: 'avif',
       width: 1280,
       height: 853,
       filename: 'dog_large.avif', // highlight-line
...
async function createImages(src) {
  const metadata = await Image(src, {
      widths: [320, 640, 1280],
      formats: ["avif", "webp", "jpeg"], 
      outputDir: './dist/img' // highlight-line
    });
}  
```
...
```JavaScript
async function imageShortcode(src, alt, sizes='', caption='') {
  let imgSrc = src; // highlight-line

  // handle same folder images, append the input path to make the path relative 
  // to project folder as 11ty requires it
  if (!imgSrc.startsWith('.')) {   // highlight-line
    const inputPath = this.page.inputPath;  // highlight-line
    const pathParts = inputPath.split('/');  // highlight-line
    pathParts.pop();  // highlight-line
    imgSrc = pathParts.join('/') + '/' + src; // highlight-line
  }

  // create images and return the files metadata
  const metadata = await Image(imgSrc, { // highlight-line
    widths: [320, 640, 1280],
    formats: ["avif", "webp", "jpeg"]
  });
...
![Mobile version is a detail of the original](./detail.jpg "Mobile version is a detail of the original")
