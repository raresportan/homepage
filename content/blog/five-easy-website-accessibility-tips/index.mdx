...
were particulary useful for me, I've learn a few things and used some of them to improve the accesibility of this site. 
...
For the site name I've used a `h1` on the index and a `h3` on the blog post pages.
The problem is that is not correct on the blog post since the page headigs are in the wrong order: h3, h1, h2.
To fix is I've replaced the `h1` and `h3` with a simple `div`.
